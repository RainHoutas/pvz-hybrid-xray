...
import { CloudFog, AlertTriangle, Wind, MousePointerClick } from 'lucide-react';
import { StandardLevel } from '@/types/level';
import { LevelHeader, LevelTalk, SeedBankBoard, LawnGrid, VasePoolPanel } from '@/components/LevelShared';

interface VaseTemplateProps {
    levelData: StandardLevel;
    metadataInfo?: any;
}

export function VaseTemplate({ levelData, metadataInfo }: VaseTemplateProps) {
    return (
        <div className="w-full flex flex-col xl:grid xl:grid-cols-[22.5rem_minmax(0,1fr)_22.5rem] gap-4 lg:gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 mt-2 xl:h-[calc(100vh-6rem)]">
                ...
                <div className="flex flex-col gap-6 h-full">
                    ...
                    <div className="mt-2 flex-1 min-h-[25rem] flex flex-col">
                        <LawnGrid
                            ...
                            vaseManager={levelData.VaseManager}
            ...
            {/* 右侧：资源池面板 */}
            <aside className="flex flex-col xl:overflow-y-auto min-h-0 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-gray-200 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
                <VasePoolPanel vaseManager={levelData.VaseManager} />
            </aside>
        </div>
    );
}
